...
def resultado_label(parent) -> tk.Label:
    ...
    return tk.Label(parent, text="", bg=COLOR_PANEL, fg=COLOR_TEXTO, font=FUENTE_LABEL, wraplength=480, justify="left")
...
def construir_tab_registrar(tab):
    ...
    estilo_label(tab, "Registrar nuevo producto", bold=True).grid(row=0, column=0, columnspan=2, pady=(20, 10), padx=20, sticky="w")
 
    # Código próximo (solo lectura, informativo)
    estilo_label(tab, "Código asignado:").grid(row=1, column=0, sticky="w", **pad)
    from datos import cargar_inventario, generar_codigo
    codigo_preview = generar_codigo(cargar_inventario())
    lbl_codigo = tk.Label(
        tab,
        text=codigo_preview,
        bg=COLOR_ENTRADA,
    ...
    lbl_codigo.grid(row=1, column=1, sticky="w", padx=20, pady=6)
    ...
    # Precio
    estilo_label(tab, "Precio unitario (S/.) *").grid(row=3, column=0, sticky="w", **pad)
    ...
    def actualizar_codigo_preview():
        """Refresca el código que se mostrará (por si alguien registró mientras tanto)."""
...
def construir_tab_buscar(tab):
    ...
    estilo_label(tab, "Buscar producto", bold=True).grid(row=0, column=0, columnspan=3, pady=(20, 10), padx=20, sticky="w")
    ...
    def estilo_radio(parent, texto, valor):
        return tk.Radiobutton(
            parent, text=texto, variable=var_modo, value=valor,
            bg=COLOR_PANEL, fg=COLOR_TEXTO, selectcolor=COLOR_ACENTO,
            activebackground=COLOR_PANEL, activeforeground=COLOR_ACENTO2,
            font=FUENTE_LABEL, command=lambda: cambiar_modo(),
        )
 
    estilo_radio(frame_radio, "Buscar por Código", "codigo").pack(side="left", padx=(0, 20))
    ...
    def mostrar_producto(producto):
        ...
        tk.Label(tarjeta, text=f"[{producto['codigo']}] {producto['nombre']}",
                 bg=COLOR_ENTRADA, fg=COLOR_ACENTO2, font=("Segoe UI", 11, "bold")).pack(anchor="w")
        tk.Label(tarjeta, text=f"Precio: S/. {producto['precio']:.2f}   |   Stock: {producto['stock']} unid.",
                 bg=COLOR_ENTRADA, fg=COLOR_TEXTO, font=FUENTE_LABEL).pack(anchor="w", pady=(2, 0))
...
def construir_tab_stock(tab):
    ...
    estilo_label(tab, "Actualizar stock", bold=True).grid(row=0, column=0, columnspan=2, pady=(20, 10), padx=20, sticky="w")
    ...
    for texto, valor in [("Entrada (+)", "entrada"), ("Salida (−)", "salida")]:
        tk.Radiobutton(
            frame_op, text=texto, variable=var_op, value=valor,
            bg=COLOR_PANEL, fg=COLOR_TEXTO, selectcolor=COLOR_ACENTO,
            activebackground=COLOR_PANEL, font=FUENTE_LABEL,
    ...
    def actualizar():
        ...
        if resultado["exito"]:
            lbl_resultado.config(text=f"✔  {resultado['mensaje']}", fg=COLOR_EXITO)
            lbl_stock.config(text=f"Stock nuevo: {resultado['stock_nuevo']} unidades", fg=COLOR_ACENTO2)
            ent_cantidad.delete(0, tk.END)
        else:
            lbl_resultado.config(text=f"✘  {resultado['mensaje']}", fg=COLOR_ERROR)
    ...
    estilo_boton(frame_botones, "Actualizar", actualizar).pack(side="left", padx=(0, 10))
    estilo_boton(frame_botones, "Limpiar", limpiar, color="#555577").pack(side="left")
...
def construir_tab_eliminar(tab):
    ...
    estilo_label(tab, "Eliminar producto", bold=True).grid(row=0, column=0, columnspan=2, pady=(20, 10), padx=20, sticky="w")
    estilo_label(tab, "Ingresa el código del producto que deseas eliminar.", dim=True).grid(
    ...
    estilo_boton(tab, "Eliminar", intentar_eliminar, color="#c0392b").grid(row=3, column=0, columnspan=2, padx=20, pady=10, sticky="w")
...
def construir_tab_listar(tab):
    ...
    estilo_boton(encabezado, "↻ Actualizar", lambda: cargar_lista(), color="#555577").pack(side="right")
 
    lbl_estado = tk.Label(tab, text="", bg=COLOR_PANEL, fg=COLOR_TEXTO_DIM, font=FUENTE_LABEL)
    ...
    encab = {"codigo": "Código", "nombre": "Nombre", "precio": "Precio (S/.)", "stock": "Stock"}
